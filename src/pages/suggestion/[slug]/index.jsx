...
export default function CreateReport() {
    ...
    async function handleComplaint(e) {
        e.preventDefault();
        const suggestion = new Parse.Object("Suggestion");

        suggestion.set("name", e.target[0].value);
        suggestion.set("content", e.target[1].value);
        suggestion.set("userPtr", Parse.User.current().toPointer());
        suggestion.set("status", "на рассмотрении");
        suggestion.set(
            "department",
        ...
        try {
            ...
            fileList.map(async (file) => {
                ...
                FileObj.set("suggestionPtr", suggestion.toPointer());
                await FileObj.save();
            ...
            try {
                ...
                notifications.set("type", "suggestion");
                notifications.set("status", "на рассмотрении");
                notifications.set("suggestionPtr", suggestion.toPointer());
                notifications.set("content", e.target[1].value);
                notifications.set("isRead", false);
                await notifications.save();

                const notification2 = new Parse.Object("Notifications");
                const userQuery = new Parse.Query(Parse.User);
                userQuery.equalTo(
                    "objectId",
                    departments.find((dep) => dep.slug === "cos").objId,
                );
                const cosPtr = (await userQuery.first()).toPointer();
                notification2.set("userPtr", cosPtr);
                notification2.set("type", "предложение");
                notification2.set("status", "на рассмотрении");
                notification2.set("suggestionPtr", suggestion.toPointer());
                notification2.set("content", e.target[1].value);
                notification2.set("isRead", false);
                await notification2.save();
            } catch (error) {
                message.error("Что-то пошло не так");
            }
        }
        // mulitple file upload to File table
    }
        ...
        <>
            <Layout>
                ...
                <Layout>
                    ...
                    <Content className={montserrat.className + " bg-uibGreen opacity-75"}>
                        <div className={"page-wo-logo page-slug"}>
